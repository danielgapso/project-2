const getData = () => {
  //this function shows the cards with the info for each card
  $("#container").html(""); //sets the container div as empty string
  for (let counter = 0; counter < 100; counter++) {
    //one loop to show 100 cards
    ...
    <div class="card" style="width: 18rem">
      ...
      onclick="addToWatchList(this, '${allCoins[counter].id}', document.querySelector('#coin-${allCoins[counter].id}'))">
        ...
        <h5 class="card-title">Coin ID : ${allCoins[counter].symbol}</h5>
        <p class="card-text">Coin Name : ${allCoins[counter].id}</p>
...
const printinfo = (id, data) => {
  //when more info is click this function is called it gets the id of the clicked card and the data
  ...
  $("#coinInfo-" + id).append(`
        <div>
          ...
          <p class="card-text"> price in dollars: ${data.market_data.current_price.usd} $</p>
          <p class="card-text"> price in euros: ${data.market_data.current_price.eur} €</p>
          <p class="card-text"> price in shekels: ${data.market_data.current_price.ils} ₪</p>
        </div>
      </div>
    </div>
    `);
